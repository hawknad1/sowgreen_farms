...
//     const cappedProductCount = Math.min(rawProductCount, 18)
...
//     const templateKey =
//       rawProductCount > 18 ? "13var_btn" : `${requiredVarCount}var`
...
export async function POST(req: NextRequest) {
  try {
    ...
    // 2. Calculate product count for padding purposes
    const rawProductCount = order.products.length
    const cappedProductCount = Math.min(rawProductCount, 18)

    // 3. Prepare order details FIRST (to get actual productLines length including headers)
    const { baseVariables, productLines, summaryValues, contactValues } =
      prepareOrderPickupDetails(order, shipping, cappedProductCount, workers)

    // 4. Calculate ACTUAL variable count using productLines.length
    const baseVarCount = 5 // [name, orderNumber, pickup_date, pickup_address, phone]
    const summaryVarCount = 5 // [subtotal, delivery, credit, total, due]
    const contactVarCount = 2 // [contact1, contact2]
    const requiredVarCount =
    ...
    // 5. Get template SID based on ACTUAL variable count
    const TEMPLATE_MAP = getTemplateMapPickupFromBase64()

    // Use button template if variable count exceeds your largest non-button template
    const templateKey =
      requiredVarCount > 30 ? "13var_btn" : `${requiredVarCount}var`
    ...
    const allVariables = [
      ...
      ...(templateKey === "13var_btn"
        ...
        : productLines), // Multiple lines with headers for other templates
      ...summaryValues, // 5 variables
      ...contactValues, // 2 variables
      ...(templateKey === "13var_btn" ? [order.id] : []), // Button ID only for button template
    ]

    // 7. Validate variable count matches template expectations
    const expectedVarCount =
      templateKey === "13var_btn"
    ...
    // 8. Create Twilio variables mapping
    const twilioVariables = Object.fromEntries(
      allVariables.map((value, index) => [(index + 1).toString(), value])
    )

    // 9. Send message with validation
    const message = await client.messages.create({
      from: `whatsapp:${whatsappNumber}`,
      to: `whatsapp:${order.userWhatsappOptIn.customerPhone}`,
      contentSid,
