...
async function fetchWorkers(): Promise<Staff[]> {
  try {
    ...
    const workers = await response.json()
    if (!Array.isArray(workers)) throw new Error("Invalid staff data format")
...
export async function POST(request: NextRequest) {
  try {
    ...
    const targetConversationSid =
      process.env.NEXT_PUBLIC_TWILIO_CONVERSATIONS_SID_SOWGREEN_FARMS
    if (!targetConversationSid) {
      throw new Error("Twilio conversation SID not configured")
    }
    ...
    const TEMPLATE_MAP = getTemplateMapFromBase64()
    if (!TEMPLATE_MAP) {
      throw new Error("Template map not loaded")
    }

    // Determine template to use
    let templateUsedKey: string
    let expectedVarCount: number
    const baseVarCount = 7
    const summaryVarCount = 5
    const contactVarCount = 2

    if (rawProductCount > CAPPED_PRODUCTS_IN_LIST) {
      templateUsedKey = "15var_btn"
      expectedVarCount = 16
    } else {
      const totalTextVars =
    ...
    const contentSid = TEMPLATE_MAP[templateUsedKey]
    if (!contentSid) {
      throw new Error(`Template ${templateUsedKey} not found in template map`)
    }

    // Prepare all message components
    const { baseVariables, productLines, summaryValues, contactValues } =
      prepareOrderDetails(
        {
          ...order,
    ...
    const allTemplateVariables: (string | number)[] =
      templateUsedKey === "15var_btn"
        ? [
            ...baseVariables.map((v) => truncate(v, 60)),
            "Click the *View Ordered Items* button below to see all purchased products.",
            ...summaryValues.map((v) => truncate(v, 60)),
            ...contactValues.map((v) => truncate(v, 60)),
            order.id || `order_${order.orderNumber}_${Date.now()}`,
          ]
        : [
            ...baseVariables.map((v) => truncate(v, 60)),
            ...productLines.map((v) => truncate(v, 60)),
    ...
    if (allTemplateVariables.length !== expectedVarCount) {
      throw new Error(`Template variable count mismatch for ${templateUsedKey}`)
    }
    ...
    const messageData: TwilioConversationsMessageCreateOptions = {
      ...
      body: `Order ${order.orderNumber} confirmation`,
    }
