...
const EditShippingDetails = ({ order, customer }: ShippingProps) => {
  const [isSaving, setIsSaving] = useState(false)
  const [selectedCity, setSelectedCity] = useState(
    order?.shippingAddress?.city || ""
  )
  ...
  const handleDeliveryFeeSave = async (newFee?: number) => {
    ...
    try {
      const feeToSave = newFee !== undefined ? newFee : deliveryFee
      const finalTotal = (order?.total || 0) + feeToSave
      const { updatedOrderTotal: newOrderTotal } = deductBalance(
        balance,
        finalTotal
      )
  ...
  useEffect(() => {
    ...
    // Check if current delivery method is pickup
    const isPickupMethod =
      deliveryMethod === "pickup" ||
      pickupLocations.some((loc) => loc.address === deliveryMethod)

    // Always set fee to 0 for pickup, regardless of manual fee status
    if (isPickupMethod) {
      setDeliveryFee(0)
      setIsManualFee(false) // Reset manual fee flag for pickup
      return
    }

    // Skip auto-calculation if fee is manually set for home delivery
    if (isManualFee) return

    // Calculate fee for home delivery
      ...
      <div className="bg-gray-50 p-4 rounded-lg border">
        ...
        {isLoadingCities || isLoadingPickupLocations ? (
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between items-center">
              <span>Delivery Fee:</span>
              {!customer && !isEditingFee ? (
                <div className="flex items-center gap-2">
                  <span>{formatCurrency(deliveryFee, "GHS")}</span>
                  <button
                    onClick={() => {
                      setTempDeliveryFee(deliveryFee.toString())
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    onClick={async () => {
                      const feeValue = Number(tempDeliveryFee) || 0
                      setIsManualFee(true)
                      await handleDeliveryFeeSave(feeValue)
                      setTempDeliveryFee("")
                      setIsEditingFee(false)
                    }}
                    disabled={isSaving}
