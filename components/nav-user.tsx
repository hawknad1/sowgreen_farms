  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      ...
      try {
        const response = await fetch(`/api/user/${user.email}`, {
          method: "GET",
          cache: "force-cache", // Use cached data if available to improve performance
        })
        if (!response.ok) throw new Error("Failed to fetch user details")
        const data = await response.json()
        setActiveUser(data)
    ...
    // Re-fetch user details when the session changes
    if (session?.user?.email !== user?.email) {
      setActiveUser(null) // Reset state to avoid stale data
      fetchUserDetails()
    }
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-[--radix-dropdown-menu-trigger-width] min-w-56">
            ...
            <DropdownMenuGroup>
              ...
              <DropdownMenuItem>
                <CreditCard className="mr-2" /> Order History
              </DropdownMenuItem>
              <DropdownMenuItem>
                <Bell className="mr-2" /> Notifications
              </DropdownMenuItem>
