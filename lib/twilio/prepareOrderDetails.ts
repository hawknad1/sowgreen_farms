...
export function prepareOrderDetails(
  ...
  // 1. Base information (7 variables)
  const baseVariables = [
    shipping.name?.split(" ")[0] || "Customer", // [1] Customer first name
    order.orderNumber, // [2] Order number
    order.deliveryDate?.split(",").slice(0, 2).join(",") || "Not specified", // [3] Delivery date
    shipping.deliveryMethod || "Home Delivery", // [4] Delivery method
    `${shipping.address}, ${shipping.city}`, // [5] Delivery address
    order.dispatchRider?.fullName || "Not assigned", // [6] Dispatch rider
    shipping.phone, // [7] Customer phone (contact)
  ]
  ...
  // 3. Summary values (5 variables)
  const summaryValues = [
    formatCurrency(order.total, "GHS"), // Subtotal
    formatCurrency(order.deliveryFee || 0, "GHS"), // Delivery fee
    formatCurrency(order.creditAppliedTotal, "GHS"), // Credit applied
    formatCurrency(order.total + (order.deliveryFee || 0), "GHS"), // Total amount
    formatCurrency(order.updatedOrderTotal, "GHS"), // Total due
  ]

  // 4. Support contacts (2 variables) - from workers, not customer
