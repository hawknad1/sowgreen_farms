...
const PRODUCT_QUANTITY_CONFIG: Record<
  string,
  {
    keywords: string[] // Changed to array for multiple keyword variations
    variants: Array<{
...
async function fetchOrders(status?: string): Promise<Order[]> {
  try {
    ...
    // Only filter by status, not by date - we'll filter by createdAt client-side
...
function parseCreatedAtDate(dateString: string): Date | null {
  try {
...
// Format date for display (e.g., "Monday, Oct 20th, 2025")
...
// Filter orders by createdAt date range
function filterOrdersByCreatedAt(
  orders: Order[],
  from: Date,
  to: Date
): Order[] {
  ...
  console.log("Filtering by createdAt date")
  ...
  const filtered = orders.filter((order, index) => {
    ...
    if (order.status !== "confirmed") {
      if (shouldLog)
        console.log(`  ✗ Status is "${order.status}", not "confirmed"`)
      return false
    }
    if (shouldLog) console.log(`  ✓ Status is "confirmed"`)

    // Ensure createdAt exists
    if (!order.createdAt) {
      if (shouldLog) console.log(`  ✗ No createdAt field`)
      return false
    }
    if (shouldLog) console.log(`  ✓ Has createdAt: "${order.createdAt}"`)

    // Parse the createdAt date
    const createdDate = parseCreatedAtDate(order.createdAt)

    if (!createdDate) {
      console.warn(`  ✗ Could not parse createdAt: "${order.createdAt}"`)
      return false
    }

    // Check if created date is within range (inclusive)
    const isInRange = createdDate >= fromDate && createdDate <= toDate
...
function groupOrdersByCreatedAt(orders: Order[]): Map<string, Order[]> {
  ...
  orders.forEach((order) => {
    ...
    const dateKey = formatDateForDisplay(createdDate)
...
function calculateProductQuantity(
  ...
  orderProducts.forEach((orderProduct) => {
    ...
    // Check for products with special unit configurations (like eggs and leaf tea)
...
function applyGrayBackgrounds(worksheet: any) {
  const grayFill = {
    type: "pattern",
    pattern: "solid",
    fgColor: COLORS.gray,
...
function applyBorders(worksheet: any, products: Product[]) {
  const totalRows = worksheet.rowCount
  const totalColumns = products.length + 3
...
// Create a worksheet for a specific date
function createWorksheetForDate(
  ...
  // Create worksheet with date as name (sanitize for Excel)
  ...
  applyGrayBackgrounds(worksheet)
...
export default async function exportConfirmedOrders(from: Date, to: Date) {
  const currentDate = formatDate(new Date())

  // Fetch data - get ALL confirmed orders, then filter by createdAt client-side
  const products = await fetchProducts()
  const allOrders = await fetchOrders("confirmed")

  console.log(`\nReceived ${allOrders.length} confirmed orders from API`)
  console.log(
    "Sample createdAt dates:",
    allOrders.slice(0, 5).map((o) => o.createdAt)
  )

  // Filter orders by createdAt and ensure status is 'confirmed'
  const orders = filterOrdersByCreatedAt(allOrders, from, to)

  // Debug logging
  console.log("=== EXPORT SUMMARY ===")
  console.log(`Total orders fetched from API: ${allOrders.length}`)
  console.log(`Filtered confirmed orders in range: ${orders.length}`)
  console.log(
  ...
  if (allOrders.length > 0) {
    ...
    allOrders.slice(0, 3).forEach((order) => {
      console.log(
        `  - ${order.orderNumber}: status="${order.status}", createdAt="${order.createdAt}"`
      )
    })
  }

  if (orders.length === 0) {
    console.warn("❌ No confirmed orders found in the specified date range")

    // Show how many orders were excluded and why
    const statusIssues = allOrders.filter(
      (o) => o.status !== "confirmed"
    ).length
    const noDateIssues = allOrders.filter((o) => !o.createdAt).length
    const dateRangeIssues = allOrders.length - statusIssues - noDateIssues

    let message = `No confirmed orders found between ${from.toLocaleDateString()} and ${to.toLocaleDateString()}.\n\n`
    message += `Total orders fetched: ${allOrders.length}\n`
    if (statusIssues > 0)
      message += `- ${statusIssues} orders with status ≠ "confirmed"\n`
    if (noDateIssues > 0)
      message += `- ${noDateIssues} orders without createdAt\n`
  ...
  // Group orders by createdAt date
  const groupedOrders = groupOrdersByCreatedAt(orders)
  console.log(`\n📊 Orders grouped into ${groupedOrders.size} date(s):`)
  groupedOrders.forEach((ordersForDate, dateLabel) => {
    console.log(`  - ${dateLabel}: ${ordersForDate.length} orders`)
  })
  ...
  groupedOrders.forEach((ordersForDate, dateLabel) => {
    console.log(
      `  → Creating tab for "${dateLabel}" with ${ordersForDate.length} orders`
    )
  ...
  const link = document.createElement("a")
  link.href = URL.createObjectURL(blob)
  link.download = `Confirmed_Orders_${currentDate.replace(/\//g, "-")}.xlsx`
  document.body.appendChild(link)
  link.click()
  document.body.removeChild(link)
}
